...
func voiceMessageHandle(ctx *openwechat.MessageContext) {
	...
	logger.Log.Infof("[收到新语音消息] == 发信人：%v ==> 内容：%v", senderUser, ctx.Content)
	msg := ctx.Message
	bot := ctx.Bot
	var resp = CallbackRes{Type: global.MSG_VOICE, MsgId: msg.MsgId, From: sender.UserName, NickName: sender.NickName, Content: msg.Content}
